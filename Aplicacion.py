# -*- coding: utf-8 -*-
"""
Created on Tue Jun  9 20:24:46 2020

@author: Patricia
"""
import cx_Oracle

def consultaPedido(conexion):
    ...
    for result in cursor:
        print(result)
    cursor.close()
def consultaComprador(conexion):
    cursor=conexion.cursor()
    consulta="select id from compradores"
    cursor.execute(consulta)
    for result in cursor:
        print(result)
    cursor.close()
def consultaFarmaco(conexion):
    cursor=conexion.cursor()
    consulta="select id, fabricantes_cif from farmacos"
    cursor.execute(consulta)
    for result in cursor:
        print(result)
    cursor.close()
def consultaFabricante(conexion):
    cursor=conexion.cursor()
    consulta="select cif from fabricantes"
    cursor.execute(consulta)
    for result in cursor:
        print(result)
    cursor.close()
def consultaEnfermedad(conexion):
    cursor=conexion.cursor()
    consulta="select nombre from enfermedades"
    cursor.execute(consulta)
    for result in cursor:
        print(result)
    cursor.close()
def insertRelacion(conexion):
    ...
    print("2. Insertar en la relación formado_por ")
    ...
    while opc !=5:
        if opc == 1:
            cursor = conexion.cursor()
            print("pedido")
            consultaPedido(conexion)
            print("compradores")
            consultaComprador(conexion)
            ...
            cursor = conexion.cursor()
            print("farmacos")
            consultaFarmaco(conexion)
            print("Fabricantes")
            consultaFabricante(conexion)
            print("pedido")
            consultaPedido(conexion)
            ...
            inserto = "INSERT INTO formado_por (farmacos_id, farmacos_cif, pedidos_id, cantidad) VALUES(" + farmacos_id + ", " + farmacos_cif + ", " + pedidos_id + ", "+ cantidad + ")"
            print(inserto)
            cursor.execute(inserto)
            cursor.close()
        elif opc == 3:
